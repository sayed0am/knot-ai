...
knot is a framework for non-coding agents, so this keeps only the message
roles a non-coding agent loop needs: ``UserMessage``, ``AssistantMessage``,
``ToolResultMessage``, and ``CustomMessage``.

These models later serialize verbatim to SSE, so the ``WireModel`` base
(camelCase JSON aliases, ``extra="forbid"``) is strict and stable by design.
"""

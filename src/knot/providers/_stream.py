"""Translate an adapter's internal provider-event stream into wire events.

Handles ``ProviderAbortedEvent`` (see ``_provider_events.py``): cancellation
is surfaced as ``AssistantErrorEvent(reason="aborted")`` rather than falling
through to the generic "stream ended without a terminal event" fallback.
"""
...
async def canonicalize_provider_stream(
    ...
    *,
    ...
    """Canonicalize an adapter's internal event stream into wire events."""
    ...
    async for event in source:
        if isinstance(event, ProviderRetryEvent):
            # Retries are provider-internal; they never reach the wire.
            continue
        ...
        if isinstance(event, ProviderTextDeltaEvent):
            ...
            terminal = True
        elif isinstance(event, ProviderAbortedEvent):
            async for end_event in _end_active_block(partial, active_index):
                yield end_event
            active_index = None
            active_kind = None
            ...
            error.error_message = "Request was cancelled"
